...
Route::get('/home','Site\SanphamController@index');
...
Route::get('tim-san-pham/search_site/{search_site?}', 'Site\SanphamController@postdata');
Route::post('tim-san-pham/search_site/{search_site?}', 'Site\SanphamController@getdata');
...
Route::get('mycart','Site\OrderController@index');
Route::post('mycart/add','Site\OrderController@add');
Route::post('mycart/update','Site\OrderController@update');
Route::get('mycart/del/{id}','Site\OrderController@delete');
...
Route::post('/comments','Site\OrderController@storekh');
...
Route::post('category/{id}', 'Admin\CatController@show'); 

Route::get('get-fillter','Site\SanphamController@test1');
Route::post('get-fillter','Site\SanphamController@test2');

    Route::get('product/{skin_type}/{skin_concerns?}/{products_type?}', 'Site\SanphamController@fill_products_get');
	
	
    Route::post('product/search/{search?}','Site\SanphamController@fill_products_post');
...
Route::group(['prefix' => 'admin','middleware' => 'admin'], function()
{
	...
	Route::get('sinhvien','Admin\SonController@index');
	Route::get('sinhvien/create','Admin\SonController@create');
	Route::post('sinhvien/create','Admin\SonController@store');
	Route::get('sinhvien/{id}/edit','Admin\SonController@edit');
	Route::post('sinhvien/{id}/edit','Admin\SonController@update');
	Route::get('sinhvien/{id}','Admin\SonController@show');
	Route::get('delete/{id}/', [ 'as' => 'DeteleStudent', 'uses' => 'Admin\SonController@destroy' ] );
    ...
    Route::resource('menus', 'Admin\MenusController');
    Route::resource('menusitem', 'Admin\MenusitemController');
    Route::resource('sinhvien', 'Admin\SonController');
	Route::resource('delete', 'Admin\SonController');
    Route::get('news','Admin\NewsController@index');
...
Route::get('{cat_alias?}',function($cat_alias=null){ //chú ý đây là hàm-viết- duong dẫn của danh mục và bài viết
    ...
    if(count($cat)) {
        ...
        $list_articles_cat_all = DB::table('product')->where('cat_id', 'LIKE', "%-".$cat->id."-%")->where('status_product','=',1)->orderBy('id', 'DESC')->get();
        $list_articles_cat = DB::table('product')->where('cat_id', 'LIKE', "%-".$cat->id."-%")->orderBy('id', 'ASC')->where('status_product','=','1')->paginate(12);
        ...
        if($product){
			...
			$category = DB::table('cat') ->whereIn('id',$categories)->get();
            //exit();
            //$cat=App\Cat::find('-'.$product->cat_id.'-');
         ...
         //var_dump($san_pham_lien_quan);
            ...
            return view('site.single')->with('product',$product)->with('title',$title)
			->with('san_pham_lien_quan',$san_pham_lien_quan)->with('cat',$cat)->with('cats',$cats)
            ...
            if($new) {
                ...
                //echo $title;
                $cat_new=App\Catnews::find($new->cat_new_id);
				$title_cat = $cat_new -> name;
				$cat_alias = $cat_new -> cat_new_alias;
                $list_new_lq=DB::table('news')->where('id','!=',$new->id)->take(4)->orderBy('id', 'DESC')->get();
                return view('site.new')->with('new',$new)->with('active',$active)->with('cat_new',$cat_new)->with('title',$title)->with('list_new_lq',$list_new_lq)
				-> with('new_alias', $new_alias) -> with('cat', $cat) -> with('title_cat', $title_cat) -> with('cat_alias', $cat_alias);
            }else {
                ...
                if(count($cat_new)) {
                    ...
                    // $san_pham_lien_quan=DB::table('news')->where('id','=',$new->id)->where('id','!=',$new->id)->where('status_new','=','0')->orderBy('id', 'DESC')->take(6)->get();
                    // var_dump($san_pham_lien_quan);die();
                    return view('site.cat_new')->with('news',$news)->with('title',$title)->with('list_new',$list_new) -> with('cat_alias', $cat_alias);
