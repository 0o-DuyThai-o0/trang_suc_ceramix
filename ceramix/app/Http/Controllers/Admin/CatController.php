...
class CatController extends Controller {
	...
	public function index( Request $request)
	{
		$session_value='0';
		$cats1 = App\Cat::all();
		$cats2 = DB::table('cat')->get();
		$catsfull =App\Cat::all();
		$all_cat = count(DB::table('cat')->get());
	...
	//	exit();
		...
		return view('admin.cat.index')->with('catsfull',$catsfull)->with('all_cat_count',$all_cat)->with('cats',$cats)->with('cats1',$cats1)->with('cats2',$cats2)->with('session_value',$session_value)->with('cat_parents',$cat_parents)->with('product_cat',$product_cat);
	...
	{ 	
		...
		$img_avatar = $request->input('add_images_create_cat');
		$img_avatar = "[".$img_avatar."]";
		$cat->category_avatar = $img_avatar;
		$img_banner =  $request->input('add_images_create_cat_danhmuc');
		$img_banner = "[".$img_banner."]";
		$cat->banner_category = $img_banner;
	...
	//	exit();
			...
			for($i=0;$i<10;$i++){
				$k='image_create_cat'.$i;
				unset($_SESSION[$k]);
			}
						for($i=0;$i<10;$i++){
				$k='image_create_cat_danhmuc'.$i;
				unset($_SESSION[$k]);
			}
		}
		return redirect('admin/cat');
	...
	public function update(Request $request,$id)
	{
		...
		$cat->cat_alias=strtolower(convert_vi_to_en($request->input('name')));
		$cat->parent_id =$request->input('parent_id');
		$img_avatar = $request->input('image_cat_edit_avatar');
		$img_avatar = "[".$img_avatar."]";
		$cat->category_avatar = $img_avatar;
		$img_banner =  $request->input('image_cat_edit_banner');
		$img_banner = "[".$img_banner."]";
		$cat->banner_category = $img_banner;
	...
	//	exit();
			...
			for($i=0;$i<10;$i++){
				$k='image_edit_category_avatar_cat'.$i;
				unset($_SESSION[$k]);
			}
						for($i=0;$i<10;$i++){
				$k='image_edit_category_banner_cat'.$i;
				unset($_SESSION[$k]);
			}
		}
	...
	public function get_s_search($s_search) {
		...
		return view('admin.cat.index')->with('catsfull',$catsfull)->with('all_cat_count',$all_cat)->with('cats2',$cats2)->with('cats',$cats)->with('cats1',$cats1)->with('session_value',$session_value)->with('cat_parents',$cat_parents)->with('s_search',$s_search)->with('products',$products);	
	}
...
//exit();
	...
	public function xoa_anh_dai_dien_cat_danh_muc($id,$number) {
		//echo $id."---".$number;
		$cat = App\Cat::find($id);
		...
		if(count($img_xs)) {
		$cat->banner_category=json_encode($img_xs); //array to json
	//	echo $cat->category_avatar;
	//	exit();
		}else $cat->banner_category='';
			if($cat->save()) {
				return redirect()->back();
			}
